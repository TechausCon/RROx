...
Scanner::Scanner(const std::string& pattern) : pattern{} {
    this->pattern = move(buildPattern(pattern));
}

std::optional<uintptr_t> Scanner::find(uintptr_t start, size_t length, bool scanCodeOnly) {
...
std::vector<int16_t> Scanner::buildPattern(std::string patternStr) {
